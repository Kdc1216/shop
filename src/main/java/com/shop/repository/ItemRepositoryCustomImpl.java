...
public class ItemRepositoryCustomImpl implements ItemRepositoryCustom{
    ...
    private BooleanExpression searchCategoryEq(Category searchCategory){
        return searchCategory == null ?
                null : QItem.item.category.eq(searchCategory);
        //ItemSellStatus null이면 null 리턴 null 아니면 SELL, SOLD 둘중 하나 리턴
    }
    ...
    @Override
    public Page<MainItemDto> getSingle(ItemSearchDto itemSearchDto, Pageable pageable){
        QItem item = QItem.item;
        QItemImg itemImg = QItemImg.itemImg;
        itemSearchDto.setSearchCategory(Category.SINGLE); // enum값을 세팅해줌
        ...
        QueryResults<MainItemDto> results = queryFactory.select(new QMainItemDto(item.id, item.itemNm,
                ...
                // join 내부 조인 .repImgYn.eq("Y") 대표이미지만 가져온다.
                .from(itemImg).join(itemImg.item, item).where(itemImg.repImgYn.eq("Y"))
                .where(searchCategoryEq(itemSearchDto.getSearchCategory())) //세팅 된 값을 넣어준다.
                .orderBy(item.id.desc()).offset(pageable.getOffset()).limit(pageable.getPageSize()).fetchResults();
        List<MainItemDto> content = results.getResults();
        long total = results.getTotal();
        return new PageImpl<>(content, pageable, total);
    ...
    @Override
    public Page<MainItemDto> getSet(ItemSearchDto itemSearchDto, Pageable pageable){
        QItem item = QItem.item;
        QItemImg itemImg = QItemImg.itemImg;
        itemSearchDto.setSearchCategory(Category.SET);
